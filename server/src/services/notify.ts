...
/**
 * Creates an in-app notification and pushes it to the person's signed-in devices. Never throws: a
...
export async function notify(userId: bigint | null | undefined, type: NotificationType, title: string, body: string, data?: Record<string, unknown>) {
  if (!userId) return;
  const saved = await prisma.notification
    .create({ data: { userId, type, title, body, dataJson: data ? (data as Prisma.InputJsonValue) : Prisma.JsonNull } })
    .catch(() => null);
  void sendPush(userId, title, body, { ...data, type, notificationId: saved ? Number(saved.id) : undefined });
}
