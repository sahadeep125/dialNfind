...
/** What the API answers to a successful sign-in, or its error body. */
...
export async function signInThroughApi(request: Request, apiPath: string, body: string, pick: (data: SignInResponse) => Record<string, unknown>) {
  ...
  const data = await readJson<SignInResponse>(res);
