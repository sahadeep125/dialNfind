...
/** A row of photo thumbnails with an add tile; each file uploads on its own as soon as it is picked. */
export function PhotoListUpload({ value, onChange, purpose, max, id, onUploadingChange }: { value: string[]; onChange: (urls: string[]) => void; purpose: UploadPurpose; max: number; id?: string; onUploadingChange?: (uploading: boolean) => void }) {
  const autoId = useId();
  const inputId = id ?? autoId;
  const input = useRef<HTMLInputElement>(null);
  const [progress, setProgress] = useState<number | null>(null);
  ...
  async function handle(files: FileList | null) {
    ...
    try {
      for (const file of picked) {
        ...
        setProgress(0);
        added.push(await uploadFile(file, purpose, setProgress));
      }
    } catch (err) {
      setError(err instanceof Error ? err.message : "Upload failed");
    } finally {
  ...
  return (
    <div>
      <input ref={input} id={inputId} type="file" multiple accept={rule.types.join(",")} className="sr-only" disabled={progress !== null} aria-describedby={error ? `${inputId}-upload-error` : `${inputId}-upload-hint`} onChange={(e) => handle(e.target.files)} />
      <div className="flex flex-wrap gap-2">
        ...
        {value.length < max && (
          <button
            ...
            className="flex size-20 cursor-pointer flex-col items-center justify-center gap-1 rounded-lg border-2 border-dashed text-xs text-muted-foreground transition-colors hover:border-primary/50 hover:bg-muted/50 disabled:cursor-wait"
          >
            {progress !== null ? (
              <>
                <Loader2 className="size-5 animate-spin text-primary" /> {progress}%
              </>
            ) : (
              <>
                <ImagePlus className="size-5 text-primary" /> Add photo
              </>
            )}
          </button>
        )}
